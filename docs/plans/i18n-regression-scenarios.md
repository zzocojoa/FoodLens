# I18N-P1-T012 다국어 회귀 시나리오 (EN/KO/JA/ZH)

작성일: 2026-02-14  
taskID: `I18N-P1-T012`

## 목적
- 핵심 사용자 플로우에서 EN/KO/JA/ZH 전환 시 문자열 혼재, 키 누락, 하드코딩 문자열 회귀를 조기에 차단한다.
...
- 실행 명령:
  ...
  - `npm run i18n:verify`
- 검증 범위:
  - 리소스 키 정합성(`en.json` vs `ko.json`, `ja-JP.json`, `zh-Hans.json`)
  - 핵심 플로우 화면/컴포넌트 내 직접 UI 문자열 하드코딩 탐지
  - 설정 언어 선택지(`auto`, `ko-KR`, `en-US`, `ja-JP`, `zh-Hans`) 노출 회귀
...
- 절차:
  ...
  4. 프로필에서 언어를 `Japanese (ja-JP)`로 변경
  5. 프로필에서 언어를 `Chinese (zh-Hans)`로 변경
  6. 동일 경로 재진입
- 기대 결과:
...
- 절차:
  ...
  3. 언어를 `ja-JP`로 설정 후 분석 1회
  4. 언어를 `zh-Hans`로 설정 후 분석 1회
  5. 요약/경고 영역 확인
- 기대 결과:
