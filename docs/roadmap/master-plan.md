...
- 작업:
  ...
  - 릴리스 체크리스트 운영
  - 내부 테스트 트랙 배포 증적 운영(Android AAB, iOS IPA/TestFlight)
  - Android 제출: 최초 1회 수동 허용 후 `eas submit` 자동 제출/재시도 절차 고정
  - 실디바이스 런타임 로그에서 MMKV 초기화 경고 재확인 및 영향도 판정 기록
- 완료 기준:
  - 릴리스 후보 2회 연속 치명 이슈 0건
  - 내부 테스트 트랙 증적 + submit 로그 + MMKV 재확인 결과가 릴리스 증적에 포함됨
