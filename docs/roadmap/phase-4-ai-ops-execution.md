...
- Cost guardrail
  ...
  - Render live env parity check:
    ...
    - 기본 검증 범위는 Google OAuth prompt, AI 모델명, label fallback, Pro fallback guardrail key, auth/analysis rate limit policy key로 제한한다.
    ...
    - `ANALYSIS_RATE_LIMIT_BACKEND=postgres`, `ANALYSIS_RATE_LIMIT_TABLE=analysis_rate_limit_events`, endpoint별 analysis limit 값이 세 서비스 live env에 모두 존재해야 한다.
...
- Observability 메타
  ...
  - Analysis rate limit: 429 차단은 `[RateLimit] blocked`, 저장소 장애는 `[RateLimit] storage unavailable` 로그로 추적한다.
