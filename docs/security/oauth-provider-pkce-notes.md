...
- FoodLens 현재 Kakao bridge는 `state`를 서버 pending backend에 저장하고 callback에서는 존재/만료/provider/app redirect URI를 검증하며, session 발급 직전 POST에서 one-time consume한다.
...
- Kakao token exchange body에 pending state의 `code_verifier`가 포함되는지 mock/stub으로 테스트한다.
