...
- Production auth-state posture was verified on 2026-05-31 through Render API metadata plus `backend/scripts/ci_auth_state_backend_smoke.py`: `foodlens-api` currently runs one instance, has `DATABASE_URL` set, uses `AUTH_STATE_BACKEND=postgres`, has `AUTH_TOKEN_HASH_SECRET` set, uses `AUTH_STATE_TABLE=auth_runtime_state`, and `AUTH_STATE_KEY=default`. The live `/health/ready` endpoint returned HTTP 200 with `ready=true`, `auth_service=true`, and no issues. This verifies current production as single-instance with persisted auth state configuration. It does not prove live multi-instance consistency because production is not currently scaled beyond one instance. Evidence: `render.yaml:15-24` declares the same required keys; the runbook requires those conditions in `docs/operations/oauth-security-runbook.md:40-68`.
...
- Before increasing `foodlens-api` above one instance, has production run live multi-instance logout/refresh revoke verification across separate API instances?
...
| TM-003 | Operator error, scale-out misconfiguration | Production is scaled above one API instance without live multi-instance revoke validation, or token hash secret drifts between deploys | Logout/revoke on one runtime is not enforced by another runtime | Revoked access/refresh token may remain usable on some instances | Auth state backend, token digests, sessions | Render API verification on 2026-05-31 showed `foodlens-api` instance count 1, `AUTH_STATE_BACKEND=postgres`, `DATABASE_URL` set, `AUTH_TOKEN_HASH_SECRET` set, `AUTH_STATE_TABLE=auth_runtime_state`, `AUTH_STATE_KEY=default`; `backend/scripts/ci_auth_state_backend_smoke.py --expected-backend postgres --require-shared-state --require-single-render-instance` passed; `from_env()` selects postgres with `DATABASE_URL` and requires hash secret in `backend/modules/auth/service.py:619-666,687-719` | Current prod is single-instance, so live cross-instance revoke behavior has not been exercised | Keep instance count at 1 until live multi-instance logout/refresh revoke smoke exists; before scale-out, verify two separate API instances observe the same revoked token/session state | Preserve smoke output as release evidence; add live multi-instance revoke smoke before increasing instance count | low while instance count remains 1 | high | medium |
