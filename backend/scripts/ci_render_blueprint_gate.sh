...
def main() -> None:
    ...
    required_env_keys = [
        ...
        "ANALYSIS_JOB_EMBEDDED_WORKER_ENABLED",
    ...
    for service in (web, worker, cron):
        ...
        require(
            service["env_vars"]["GEMINI_FOOD_RETRY_MAX_OUTPUT_TOKENS"]["value"]
            == service["env_vars"]["GEMINI_FOOD_MAX_OUTPUT_TOKENS_RETRY"]["value"],
            f"{service['name']} Gemini food retry token aliases must match",
        )
        require(
    ...
    env_example_expected_values = {
        ...
        "BARCODE_ALLERGEN_ESTIMATED_TOKENS_PER_REQUEST": "500",
        "AUTH_GOOGLE_OAUTH_PROMPT": "select_account",
        "ANALYSIS_RATE_LIMIT_JOBS_PER_MIN": "15",
        "ANALYSIS_RATE_LIMIT_JOB_STATUS_PER_MIN": "90",
        "ANALYSIS_INFLIGHT_MAX_JOBS": "3",
        "ANALYSIS_JOB_EMBEDDED_WORKER_ENABLED": "0",
        "AUTH_RATE_LIMIT_ENABLED": "1",
